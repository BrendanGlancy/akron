...
-- What I had to do
SELECT ITEM_ID, DESCRIPTION, CATEGORY FROM ITEM WHERE CATEGORY IN (SELECT CATEGORY FROM ITEM GROUP BY CATEGORY HAVING COUNT(*) > 1) ORDER BY CATEGORY LIMIT 6
-- The correct answer
...
SELECT INVOICES.INVOICE_NUM, INVOICE_DATE FROM CUSTOMER JOIN INVOICES ON CUSTOMER.CUST_ID = INVOICES.CUST_ID WHERE FIRST_NAME = 'James' AND LAST_NAME = 'Gonzalez'

-- List the invoice number and invoice date for each invoice that constains an invoice line for a 'Wild Bird Food (25 lb)'
-- Join the INVOICE_LINE table to the INVOICES table to get the invoice number and the invoice number
-- Wild Bird Food (25 lb) is the description of the item
SELECT INVOICES.INVOICE_NUM, INVOICE_DATE FROM INVOICES JOIN INVOICE_LINE ON INVOICE_LINE.INVOICE_NUM = INVOICES.INVOICE_NUM JOIN ITEM ON INVOICE_LINE.ITEM_ID = ITEM.ITEM_ID WHERE DESCRIPTION = 'Wild Bird Food (25 lb)'
...
SELECT ITEM_ID, DESCRIPTION, PRICE, CATEGORY FROM ITEM WHERE PRICE > ALL (SELECT PRICE FROM ITEM WHERE CATEGORY = 'CAT')


SELECT ITEM.ITEM_ID, ITEM.DESCRIPTION, ITEM.PRICE, ITEM.CATEGORY
  FROM ITEM
 WHERE ITEM.PRICE > ALL (SELECT PRICE FROM ITEM WHERE CATEGORY = 'CAT')
...
SELECT SALES_REP.REP_ID, SALES_REP.FIRST_NAME AS REP_FIRST, SALES_REP.LAST_NAME AS REP_LAST, CUST_ID, CUSTOMER.FIRST_NAME AS CUSTOMER_FIRST, CUSTOMER.LAST_NAME AS CUSTOMER_LAST FROM SALES_REP JOIN CUSTOMER ON SALES_REP.REP_ID = CUSTOMER.REP_ID ORDER BY REP_ID
