...
def calc_flop(c4, c3, flop):
    # build all possible combinations with our present cards 
    # calculate the average score for all 5 combinations of present
    flopscore = expected_value(flop,combi)
    # current flop score
    current = df.loc[df['value'] >= flopscore[0]].index[0]/2598960*100
    # avg future flop score
    ...
    # based on the value of your current or future hand, depending which is better will tell you to fold or bet under a certain amount
    if current > future:
